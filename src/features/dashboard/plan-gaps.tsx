  ...
  return (
    <DashboardCard title="Needs a plan">
      <ul className="divide-y divide-border">
        {gaps.map((gap) => (
          <li key={gap.spokeId} className="flex min-w-0 items-baseline justify-between gap-3 py-2.5">
            <Link
              href={`/spoke/${gap.spokeId}`}
              className="min-w-0 text-sm break-words text-primary underline-offset-4 hover:underline"
            >
              {gap.spokeName}
            </Link>
